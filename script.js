// The variable which represents the table body to push data in the next step.
...
// This Function is used to filter and update table based on search input also here I have used .then method to render table data.
...
// Here I have added an Event listener for the search input
document.getElementById('search_bar').addEventListener('keyup', function(event) {
    const searchTerm = event.target.value;
    updateTable(searchTerm);
});

// This is the function to sort by Market Cap in Descending Order.
function MarketCap() {
    fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=10&page=1&sparkline=false')
    .then(response => {
        return response.json();
...
// This is the function to sort Percentage in Descending Order.
function Percentage() {
    fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=10&page=1&sparkline=false')
    .then(response => {
        return response.json();
